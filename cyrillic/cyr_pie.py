...
for i in range(1040, 1072):
    dict_of_symb.update({chr(i): list_of_comb[i - 1040]})

dict_of_symb.update({'Ё': list_of_comb[33]})

word = input().upper()

for symbol in word:
    if not symbol in dict_of_symb.keys() and symbol != ' ':
        print('Неправильные символы. Возможные: кирилиллица')
...
for i in range(len(word) - 1, -1, -1):
    if word[i] == ' ':
        ...
        draw = Wedge((0, 0), i + 2, (int(sequence[0]) - 1) * 40, (int(sequence[1]) - 1) * 40, facecolor=[0, 0, 0],
                     edgecolor=list_colors[i], linewidth=width)
        # print((int(sequence[0]) - 1) * 40, (int(sequence[1]) - 1) * 40)
        ax.add_artist(draw)

for i in range(1, 10):
    ax.add_artist(Wedge((0, 0), len(word) + 3, (i - 1) * 40, (i * 40), color='yellow', linewidth=0.15, linestyle='--', fill=False))
    plt.text((len(word) + 4) * np.cos(np.radians((i - 1) * 40)), (len(word) + 4) * np.sin(np.radians((i - 1) * 40)),
